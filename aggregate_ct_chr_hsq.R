...
filename <- paste("~/repo/methylation_residuals/res_ct_chr/results_ct_hsq", first:last, ".RData", sep="")
print(filename)
...
for(i in 1:n)
{
	...
	l7[[i]] <- l7[[i]][index,]
	stopifnot(nrow(l7[[i]]) == length(l1[[i]]))
}

save(l1, l2, l3, l4, l5, l6, l7, file=paste("~/repo/methylation_residuals/res_ct_chr/allres", jid, ".RData", sep=""))
