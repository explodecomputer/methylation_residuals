makePhen <- function(allphenfile, idfile, row, phenfile)
{
	cmd <- paste("head -n ", row, " ", allphenfile, " | tail -n 1 | tr ' ' '\n' > ", phenfile, ".temp", sep="")
	system(cmd)
	cmd <- paste("paste -d ' ' ", idfile, " ", phenfile, ".temp > ", phenfile, sep="")
	system(cmd)
	system(paste("rm ", phenfile, ".temp", sep=""))
}

getCisSnps <- function(snpinfo, probeinfo, cpg, win, snplistfile)
{
	a <- subset(probeinfo, TargetID == cpg)
	b <- subset(snpinfo, CHR == a$CHR & BP < (a$COORDINATE_36 + win) & BP > (a$COORDINATE_36 - win))
	write.table(b$SNP, file=snplistfile, row=F, col=F, qu=F)
}

makeCisGrm <- function(grmfile, plinkfile, flags, idfile, snplistfile, outfile, mgrmfile, snpinfo, probeinfo, cpg, win)
{
	getCisSnps(snpinfo, probeinfo, cpg, win, snplistfile)

	cmd <- paste("plink1.90 ", flags, " --bfile ", plinkfile, " --keep ", idfile, " --make-grm-bin --extract ", snplistfile, " --maf 0.01 --out ", outfile, sep="")
	system(cmd)
	# make mgrm file
	cmd <- paste("echo ", outfile, " > ", mgrmfile, "; echo ", grmfile, " >> ", mgrmfile, sep="")
	system(cmd)
}

runGcta <- function(mgrmfile, phenfile, outfile, flags="")
{
	cmd <- paste("gcta64 ", flags, " --mgrm ", mgrmfile, " --reml --reml-no-lrt --pheno ", phenfile, " --reml-pred-rand --out ", outfile, sep="")
	system(cmd)
}

readPreds <- function(rootname)
{
	filename <- paste(rootname, ".indi.blp", sep="")
	if(file.exists(filename))
	{
		a <- read.table(filename, colClass=c("character", "character", "numeric", "numeric", "numeric", "numeric"))
		a$probe <- basename(rootname)
		return(a)
	}
}

readHsqs <- function(rootname)
{
	filename <- paste(rootname, ".hsq", sep="")
	if(file.exists(filename))
	{
		a <- read.table(filename, he=T, fill=TRUE)
		a$probe <- basename(rootname)
		return(a)
	} else {
		return(NULL)
	}
}

removeFiles <- function(rootname)
{
	a <- paste(rootname, c(".phen", ".hsq", ".indi.blp", ".log", ".grm.id", ".grm.bin", ".grm.N.bin", ".snplist", ".mgrm"), sep="")
	unlink(a)
}

arguments <- commandArgs(T)
jid <- as.numeric(arguments[1])
nrun <- as.numeric(arguments[2])

savefile1 <- paste("~/repo/methylation_residuals/res/results_ct_hsq", jid, ".RData", sep="")
savefile2 <- paste("~/repo/methylation_residuals/res/results_ct_pred", jid, ".RData", sep="")

if(all(file.exists(c(savefile1, savefile2)))) q()

load("~/repo/methylation_residuals/data/parameters.RData")

win <- 1000000
first <- (jid - 1) * nrun + 1
last <- min(nrow(params), jid * nrun)


print(c(first, last))

# grmroot <- "~/repo/methylation_residuals/data/alspac_hm3_GROUP_unrelated_m"
# allphenroot <- "~/repo/methylation_residuals/data/ALN.tost.betas.TIMEPOINT.OCT.ranktransformed.txt"
# idroot <- "~/repo/methylation_residuals/data/GROUP.id"
# phenroot <- "~/repo/methylation_residuals/results/TIMEPOINT/CPG.phen"
# outroot <- "~/repo/methylation_residuals/results/TIMEPOINT/CPG"

preds <- list()
hsqs <- list()
nom <- list()
j <- 1
for(i in first:last)
{
	cat(i, "\n")
	outfile <- paste("~/repo/methylation_residuals/res/", params$timepoint[i], "/", params$cpg[i], sep="")
	if(!file.exists(paste(outfile, ".hsq", sep="")) & params$cpg[i] %in% probeinfo$TargetID)
	{
		plinkfile <- paste("~/repo/mQTL-partitioning/grm/alspac_hm3_", params$group[i], sep="")
		grmfile <- paste("~/repo/methylation_residuals/data/", params$timepoint[i], sep="")
		allphenfile <- paste("~/repo/methylation_residuals/data/", params$timepoint[i], "norm.phen", sep="")
		idfile <- paste("~/repo/methylation_residuals/data/", params$timepoint[i], ".id", sep="")

		cisgrmfile <- outfile
		mgrmfile <- paste(cisgrmfile, ".mgrm", sep="")
		snplistfile <- paste(cisgrmfile, ".snplist", sep="")
		phenfile <- paste(outfile, ".phen", sep="")

		makePhen(allphenfile, idfile, params$index[i], phenfile)
		makeCisGrm(grmfile, plinkfile, "", idfile, snplistfile, cisgrmfile, mgrmfile, snpinfo, probeinfo, params$cpg[i], win)
		runGcta(mgrmfile, phenfile, outfile)

		nom[[j]] <- c(params$cpg[i], params$timepoint[i])
		preds[[j]] <- readPreds(outfile)
		hsqs[[j]] <- readHsqs(outfile)
		j <- j + 1

		removeFiles(outfile)

	}
}

save(nom, hsqs, file=savefile1)
save(nom, preds, file=savefile2)
